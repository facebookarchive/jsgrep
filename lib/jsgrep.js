...
Matcher.prototype.find = function(pattern, callback) {
  var ret = false;
  var self = this;
  forEachNode(this.ast, function(node) {
    if (self._match(node, pattern, callback)) {
      ret = true;
    }
  });
...
Matcher.prototype.match = function(pattern, callback) {
  return this._match(this.ast, pattern, callback);
};

Matcher.prototype._match = function(node, pattern, callback) {
  var patternAst = Matcher.compilePattern(pattern);

  this.options.variables = {};
  if (astIsEqual(node, patternAst, this.options)) {
    if (callback) {
      var v = {};
      v.node = new Matcher(node);
      for (var i in this.options.variables) {
        if (this.options.variables.hasOwnProperty(i)) {
          v[i] = new Matcher(this.options.variables[i]);
        }
      }
      if (!callback.call(null, v)) {
        return false;
      }
    }
    if (this.options.callback) {
      this.options.callback.call(null, node, this.options.variables);
    }
    return true;
  }
};

Matcher.compilePattern = function(pattern) {
